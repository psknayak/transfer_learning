...
def create_meta_csv(dataset_path, destination_path):
    ...
    This function creates and saves a meta csv file named 'habitat_dataset.csv' given a dataset folder path of images.
    ...
    Returns:
        True (bool): Returns True if 'habitat_dataset.csv' was created successfully else returns an exception
    """

    # Change dataset path accordingly
    DATASET_PATH = os.path.abspath(dataset_path)
    print(DATASET_PATH)
    if not os.path.exists(os.path.join(DATASET_PATH, "/habitat_dataset.csv")):
        ...
        # write out as habitat_dataset.csv in destination_path directory
        dp = destination_path + '/habitat_dataset.csv'
        with open(dp, 'a') as f:
            writer = csv.writer(f)
...
def create_and_load_meta_csv_df(dataset_path, destination_path, randomize=True, split=None):
    ...
    if create_meta_csv(dataset_path, destination_path=destination_path):
        dframe = pd.read_csv(os.path.join(destination_path, 'habitat_dataset.csv'))
...
if __name__ == "__main__":
    # test config
    dataset_path = './datasets/Habitats/HabitatsDataset'
    dest = './datasets/Habitats/'
    classes = 24
    total_rows = 7582
    randomize = True
    clear = True
